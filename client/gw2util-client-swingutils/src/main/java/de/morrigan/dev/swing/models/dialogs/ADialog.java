...
public abstract class ADialog<L> extends JDialog implements IObserver {
	...
	public ADialog(final Window window, final String title, final IObservable model, final ListenerWrapper<L> listener) {
		super(window, title, ModalityType.APPLICATION_MODAL);
		createAGUI();
		layoutAGUI();
		configureGUI(model);

		unwrapAndConfigureListener(listener);

		this.setVisible(true);
	}

	public ADialog(final Window window, final String title, final ListenerWrapper<L> listener) {
	...
	/**
	 * In dieser Methode werden die Listener konfiguriert. Achtung, der Parameter kann null sein.
	 * 
	 * @param listener
	 */
	protected abstract void configureListener(final L listener);
	...
	private void unwrapAndConfigureListener(final ListenerWrapper<L> listener) {
		if (listener == null) {
			configureListener(null);
		} else {
			configureListener(listener.getListener());
		}
	}
