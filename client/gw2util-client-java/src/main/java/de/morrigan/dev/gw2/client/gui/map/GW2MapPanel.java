...
public class GW2MapPanel extends JXMapKit implements IObserver, IStructuredView, ActionListener {
  ...
  private enum ListenerAction implements IListenerAction {
    KEY_PRESSED, MOUSE_WHEEL_MOVED, MOUSE_MOVED, MOUSE_DRAGGED, MOUSE_PRESSED, MOUSE_RELEASED, MOUSE_CLICKED, MOUSE_ENTERED, MENU_ITEM_SELECTED;
  }
        ...
        case MOUSE_CLICKED:
          ...
          if (mousecEvent.getButton() == MouseEvent.BUTTON3) {
            ...
            if (this.model.getMapActionMode() == MapActionMode.SET_WAYPOINT) {
              ...
              && (this.model.getMapActionMode() == MapActionMode.SET_WAYPOINT)) {
          ...
          if (gw2Waypoint.isHoverEnabled()) {
            ...
            if (convertedGpPT.distance(mousePoint) < ROLLOVER_ACTION_SIZE) {
              if ((gw2Waypoint.getWpType() == WPType.POI) //
                ...
                createInfo.append(" ");
                createInfo.append(LANGUAGES.getLabel("by"));
                createInfo.append(" ");
  ...
  private JMenuItem getShowHideMiniMapMenuItem() {
    ...
    if (main.isMiniMapShown()) {
      ...
      menuItem.setText(LANGUAGES.getLabel("hideMiniMap"));
    } else {
      menuItem.setSelected(false);
      menuItem.setText(LANGUAGES.getLabel("showMiniMap"));
    }
  ...
  private void showCreateMenu(MouseEvent event) {
    ...
    JMenu menuOre = new JMenu(LANGUAGES.getLabel("ore"));
    JMenu menuWood = new JMenu(LANGUAGES.getLabel("wood"));
    JMenu menuPlant = new JMenu(LANGUAGES.getLabel("plant"));
    JMenu menuOther = new JMenu(LANGUAGES.getLabel("other"));
    ...
    JMenu menuOriPlant = new JMenu(LANGUAGES.getLabel("orichalcumHarvestingSickle"));
    JMenu menuMithrilPlant = new JMenu(LANGUAGES.getLabel("mithrilHarvestingSickle"));
    JMenu menuDarksteelPlant = new JMenu(LANGUAGES.getLabel("darksteelHarvestingSickle"));
    JMenu menuSteelPlant = new JMenu(LANGUAGES.getLabel("steelHarvestingSickle"));
    JMenu menuIronPlant = new JMenu(LANGUAGES.getLabel("ironHarvestingSickle"));
    JMenu menuCopperPlant = new JMenu(LANGUAGES.getLabel("copperHarvestingSickle"));
    ...
    for (int i = 0; i < entries.size(); i++) {
      ...
      JMenuItem item = new JMenuItem(LANGUAGES.getLabel(entry.label), entry.icon);
  ...
  private void showEditMenu(MouseEvent event) {
    ...
    JMenuItem itemRich = new JMenuItem(LANGUAGES.getLabel("rich"), IMAGE_MANAGER.getImageIcon(
        ImageConstants.RICH_ICON, 10, 10));
    itemRich.setActionCommand(GW2MapModel.AC_RICH_WP);
    itemRich.addActionListener(this);
    JMenuItem itemPermanent = new JMenuItem(LANGUAGES.getLabel("permanent"), IMAGE_MANAGER.getImageIcon(
    ...
    JMenuItem itemDelete = new JMenuItem(LANGUAGES.getLabel("delete"), IMAGE_MANAGER.getImageIcon(
  ...
  private void showMapEditMenu(final MouseEvent event) {
    ...
    JMenuItem itemDelete = new JMenuItem(LANGUAGES.getLabel("resetSpots"), IMAGE_MANAGER.getImageIcon(
        ImageConstants.DELETE_ICON, 10, 10));
    itemDelete.setActionCommand(GW2MapModel.AC_DELETE_WP);
